...
$stmt = $pdo->prepare("
  ...
  FROM payments
  WHERE ref_table = 'sales_invoices' AND ref_id = ?
...
<div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
  ...
  <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
    <div class="rounded-2xl border border-slate-200 p-3">
      ...
      <div class="font-extrabold"><?= h($invoice['customer_name'] ?? '') ?></div>
    </div>
        ...
        <?php else: ?>
          <?php foreach ($items as $idx => $it): ?>
            <tr class="border-b hover:bg-slate-50">
              ...
              <td class="py-2 px-2"><?= money($it['unit_price']) ?></td>
  ...
  <div class="flex items-center justify-between gap-2">
    ...
    <a href="<?= $base ?>/payment_in_create.php?customer_id=<?= (int)$invoice['customer_id'] ?>&invoice_id=<?= (int)$invoice['id'] ?>"
       class="inline-flex items-center rounded-xl border border-slate-200 px-4 py-2 hover:bg-slate-50 font-semibold">
      + تحصيل
    </a>
  </div>
  ...
  <div class="mt-3 text-xs text-slate-500">
    ملاحظة: الدفعات هنا بتظهر لو كانت مسجلة في payments مع (ref_table='sales_invoices' و ref_id=invoice_id).
  </div>
</div>
